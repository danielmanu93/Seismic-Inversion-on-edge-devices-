...
def PlotPrediction(pd,vel_rec1, vel_rec2, vel_rec3,label_dsp_dim,label_dsp_blk,dh,minvalue,maxvalue,font2,font3,SavePath):
    ...
    ax2.invert_yaxis()
    ax2.vlines(x=[vel_rec1/100, vel_rec2/100, vel_rec3/100], ymin=0, ymax=2, colors=['r', 'b', 'k'], linestyles='--')
    fig2.patch.set_facecolor("#C0C0C0") ##C5C9C7
    fig2.patch.set_edgecolor("#C0C0C0") ##C5C9C7
    plt.colorbar(im2,ax=ax2,cax=cax).set_label('Velocity (m/s)')
    plt.tick_params(labelsize=7)
    plt.subplots_adjust(bottom=0.20,top=0.65,left=0.08,right=0.95)
...
def display_seismic(seis, seis_rec1, seis_rec2, seis_rec3, data_dsp_dim, data_dsp_blk, dh, font2, font3, SavePath):
    ...
    plt.colorbar(seis,ax=ax).set_label('Pressure')
    plt.tick_params(labelsize=6)
    for label in  ax.get_xticklabels()+ax.get_yticklabels():
        label.set_fontsize(6)
    ax.set_title('Seismic Data',font3)
    ax.set_xlabel('Position (km)',font2)
    ax.set_ylabel('Time (s)',font2)
...
def plot_profile(prof1, prof2, prof3, data_dsp_dim, data_dsp_blk, dh, font2, font3, SavePath):
    fig, (ax1, ax2, ax3) = plt.subplots(figsize=(7, 5), nrows=3, sharex=True, sharey=True, subplot_kw=dict(frameon=False))
    plt.suptitle("Velocity Profile of 3 locations", fontsize=16, y=1.00)
    prof1 = np.flip(prof1)
    ...
    # fig.text(0.5, 0.001, "Depth (km)", ha="center")
    # fig.text(0.001, 0.5, "Velocity (m/s)", va="center", rotation="vertical")
    ax3.set_xlabel("Depth (km)", {'family': 'Times New Roman', 'weight':'normal', 'size':13})
    ax2.set_ylabel("Velocity (m/s)", {'family': 'Times New Roman', 'weight':'normal', 'size':13})
    fig.set_facecolor("#C0C0C0")
    fig.set_edgecolor("#C0C0C0") 
    plt.tight_layout()
    plt.subplots_adjust(hspace=0.0)

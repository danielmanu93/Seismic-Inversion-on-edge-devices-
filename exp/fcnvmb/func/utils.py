...
# Plot Prediction
def PlotPrediction(pd,label_dsp_dim,label_dsp_blk,dh,minvalue,maxvalue,font2,font3,SavePath):
...
def display_seismic(seis, data_dsp_dim, data_dsp_blk, dh, font2, font3, SavePath):
    ...
    plt.colorbar(seis,ax=ax,cax=cax).set_label('Pressure')
    plt.tick_params(labelsize=6)
    for label in  ax.get_xticklabels()+ax.get_yticklabels():
        label.set_fontsize(6)
...
def plot_trace(trace1, trace2, trace3, data_dsp_dim, data_dsp_blk, dh, font2, font3, SavePath):
    ...
    plt.suptitle("Trace of 3 receivers", y=1.00)
    ...
    fig.text(0.03, 0.5, "Pressure", va="center", rotation="vertical")
...
def plot_profile(prof1, prof2, prof3, data_dsp_dim, data_dsp_blk, dh, font2, font3, SavePath):
    fig, (ax1, ax2, ax3) = plt.subplots(nrows=3, sharex=True, sharey=True, subplot_kw=dict(frameon=False))
    plt.suptitle("Velocity Profile of 3 receivers", y=1.00)
    prof1 = np.flip(prof1)
    ...
    fig.text(0.03, 0.5, "Velocity (m/s)", va="center", rotation="vertical")
    fig.set_facecolor("#C0C0C0")
    fig.set_edgecolor("#C0C0C0") 
    plt.tight_layout()
    plt.subplots_adjust(hspace=0.0)

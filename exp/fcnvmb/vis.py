...
def plot_kimb_trace(trace1, trace2, trace3, path):
    ...
    plt.suptitle("Traces of 3 receivers", fontsize=20, y=1.00)
    ...
    ax2.set_ylabel("Amplitude", {'family': 'Times New Roman', 'weight':'normal', 'size':14})
...
def plot_kimb_profile(prof1, prof2, prof3, path):
    ...
    plt.suptitle("Velocity Profiles of 3 locations", fontsize=18, x=0.58, y=1.00)
    ...
    line_labels = [ax.get_legend_handles_labels() for ax in fig.axes]
    lines, labels = [sum(lol, []) for lol in zip(*line_labels)]
    fig.legend(lines, labels, loc=2, fontsize=11)
    ax3.set_xlabel("Depth (km)", {'family': 'Times New Roman', 'weight':'normal', 'size':13})
    ax2.set_ylabel("Velocity (m/s)", {'family': 'Times New Roman', 'weight':'normal', 'size':14})
    fig.set_facecolor("#C0C0C0")
    fig.set_edgecolor("#C0C0C0") 
    plt.tight_layout()
    plt.subplots_adjust(hspace=0.0)
